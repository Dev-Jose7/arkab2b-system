...
public interface AnalyticFactPersistencePort {
    ...
    Mono<AnalyticFact> findById(OrganizationId organizationId, FactId factId);

    Mono<AnalyticFact> findBySourceEventId(OrganizationId organizationId, SourceEventId sourceEventId);

    Flux<FactSearchProjection> search(FactSearchFilter filter);

    Mono<Long> count(FactSearchFilter filter);

    Flux<AnalyticFact> findAppliedByOrganization(String organizationId);
}
