CREATE TABLE IF NOT EXISTS analytic_facts (
    ...
    CONSTRAINT ux_facts_organization_source_event UNIQUE (organization_id, source_event_id)
);

CREATE INDEX IF NOT EXISTS idx_analytic_facts_organization_period_status
    ON analytic_facts (organization_id, period, fact_status);
CREATE INDEX IF NOT EXISTS idx_analytic_facts_organization_event_occurred
    ON analytic_facts (organization_id, event_type, occurred_at DESC);
...
CREATE TABLE IF NOT EXISTS sales_projections (
    ...
    CONSTRAINT ux_sales_organization_period UNIQUE (organization_id, period)
);

CREATE INDEX IF NOT EXISTS idx_sales_projection_period
    ON sales_projections (organization_id, period DESC);
...
CREATE TABLE IF NOT EXISTS replenishment_projections (
    ...
    CONSTRAINT ck_replenishment_projection_values
        CHECK (version >= 0),
    CONSTRAINT ux_supply_organization_period_sku UNIQUE (organization_id, period, sku)
);

CREATE INDEX IF NOT EXISTS idx_replenishment_projection_period_risk
    ON replenishment_projections (organization_id, period, risk_level);
...
CREATE TABLE IF NOT EXISTS operations_kpi_projections (
    ...
    CONSTRAINT ux_kpi_organization_period_name UNIQUE (organization_id, period, kpi_name)
);

CREATE INDEX IF NOT EXISTS idx_operations_kpi_period
    ON operations_kpi_projections (organization_id, period, kpi_name);
...
CREATE TABLE IF NOT EXISTS weekly_report_executions (
    ...
    CONSTRAINT ux_weekly_exec_organization_week_type UNIQUE (organization_id, week_id, report_type)
);

CREATE INDEX IF NOT EXISTS idx_weekly_exec_organization_status
    ON weekly_report_executions (organization_id, status, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS report_artifacts (
    ...
    CONSTRAINT ux_artifacts_organization_week_type_format UNIQUE (organization_id, week_id, report_type, format),
...
CREATE INDEX IF NOT EXISTS idx_report_artifacts_execution
    ON report_artifacts (organization_id, execution_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS consumer_checkpoints (
    ...
    CONSTRAINT ux_consumer_checkpoint_scope UNIQUE (organization_id, consumer_name, topic, partition)
);

CREATE INDEX IF NOT EXISTS idx_consumer_checkpoint_lag
    ON consumer_checkpoints (organization_id, lag DESC, updated_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_reporting_audits_idempotency
    ON reporting_audits (organization_id, action_type, idempotency_key)
    WHERE idempotency_key IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_reporting_audits_target
    ON reporting_audits (organization_id, target_type, target_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_reporting_audits_created
    ON reporting_audits (organization_id, created_at DESC);
