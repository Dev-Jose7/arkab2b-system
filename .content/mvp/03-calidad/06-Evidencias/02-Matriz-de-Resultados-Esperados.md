...
|---|---|---|---|---|---|---|---|
| IAM-UT-CRIT | FR-009, NFR-005 | `identity-access-service` | Unitarias | Listo para implementar | resultado de runner + cobertura de reglas IAM | sesion/rol/organization invalido rechazado sin side effects | Si |
...
| NFR-ORGANIZATION-ISO | NFR-005, D-CROSS-01 | servicios core | Integracion/E2E no funcional | Listo para implementar | evidencias cross-organization negativas + auditoria | cero acceso cruzado en mutaciones core | Si |
...
| NFR-SEC-TRACE | NFR-006, NFR-007 | todos | Integracion | Listo para implementar | logs/trazas/auditoria con metadatos | `traceId`/`correlationId`/`organizationId`/`actorId` presentes | Si |
