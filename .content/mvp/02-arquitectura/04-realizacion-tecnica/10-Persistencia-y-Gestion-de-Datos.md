...
| `catalog-service` | `product`, `variant`, `price`, `variant_attribute` | SKU unico por organization; precio vigente resoluble sin traslape temporal | oferta vendible y precio de catalogo | vistas de catalogo consultable |
...
| `reporting-service` (`Generic`) | `analytic_fact`, `sales_projection`, `replenishment_projection`, `weekly_report_execution`, `report_artifact` | dedupe por `sourceEventId`; unicidad por `organization+week+reportType` en ejecucion semanal | snapshots y proyecciones derivadas | reportes semanales y KPI de lectura |
...
| `catalog-service` | `products`, `variants`, `prices`, `price_schedules`, `catalog_audits`, `outbox_events`, `processed_events` | `ux_variants_organization_sku`, indice de precio vigente por `variant+effective_from`, indice de facetas `attribute_code+normalized_value`, `idx_outbox_pending_occurred` | auditoria extendida; `outbox` 30 dias; crecimiento controlado de atributos |
| `inventory-service` | `stock_items`, `stock_reservations`, `stock_movements`, `idempotency_records`, `inventory_audit`, `outbox_event`, `processed_event` | checks de no-negativo/consistencia reservada, indice de contencion `organization+warehouse+sku`, dedupe por idempotency key, `ux_processed_event_consumer` | `stock_movement` particion temporal; `processed_event` corto para dedupe |
| `order-service` | `carts`, `cart_items`, `checkout_attempts`, `purchase_orders`, `order_lines`, `payment_records`, `order_status_history`, `order_audit`, `idempotency_records`, `outbox_event` | unicidad de `checkout_correlation_id` por organization, idempotencia por `organization+operation+key`, indices por estado de pedido y fecha, historial ordenado por `occurred_at` | `order_status_history` y `order_audit` con archivado mensual |
| `notification-service` (`Generic`) | `notification_requests`, `notification_attempts`, `provider_callbacks`, `notification_audits`, `channel_policies`, `outbox_events`, `processed_events` | `ux_requests_organization_notification_key`, `ux_attempts_notification_number`, dedupe de callback `provider+providerRef+callbackEventId`, `idx_outbox_status_occurred` | `notification_attempts` mensual; callbacks 6 meses; auditoria 24 meses |
| `reporting-service` (`Generic`) | `analytic_facts`, `sales_projections`, `replenishment_projections`, `operations_kpi_projections`, `weekly_report_executions`, `report_artifacts`, `consumer_checkpoints`, `outbox_events`, `processed_events` | `ux_facts_organization_source_event`, `ux_sales_organization_period`, `ux_supply_organization_period_sku`, `ux_weekly_exec_organization_week_type`, `ux_artifacts_organization_week_type_format` | `analytic_facts` particion mensual; artifacts 12 meses; `processed_events` 60 dias |
